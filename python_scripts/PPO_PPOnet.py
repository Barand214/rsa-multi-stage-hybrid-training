...
class FourierActionSpace:
    ...
    def __init__(self, n_servos=20, max_harmonics=20, T_max=10.0):
        ...
        # 5. φ: n_servos * max_harmonics个参数 [0, 2π)（相位，弧度）
    ...
    def sample_random(self):
        ...
        # φ_i ∈ [0, 2π)（相位，弧度）
        φ = np.random.uniform(0, 2 * np.pi, (self.n_servos, self.max_harmonics))
    ...
    def get_fourier_curve(self, params, t):
        """计算在时间t的舵机角度（相位φ单位为弧度，频率ω单位为rad/s）"""
        n = int(params['n'])
        T = float(params['T'])
        A = params['A']
        ω = params['ω']
        ...
        # 使用 n 个谐波：k = 0..n-1
        # f(t) = Σ_{k=0}^{n-1} A_k * cos(ω_k*t - φ_k)
        # （不再额外加 A0/2，避免 A0 既当 DC 又当谐波导致重复/耦合）
    ...
    def batch_get_fourier_curve(self, params, t_values):
        ...
        t_values = np.array(t_values, dtype=np.float32) % T
        if np.isscalar(t_values):
            t_values = np.array([t_values], dtype=np.float32)

        batch_size = len(t_values)
        angles = np.zeros((batch_size, self.n_servos), dtype=np.float32)
        ...
        for i, t in enumerate(t_values):
            for servo_idx in range(self.n_servos):
                ...
                for harmonic in range(0, n):
                    angle += A[servo_idx, harmonic] * np.cos(
                        ω[harmonic] * float(t) - φ[servo_idx, harmonic]
                    )
...
class ActorCritic(nn.Module):
    ...
    def forward(self, x, state, x_graph):
        ...
        # === 【傅里叶模式】输出傅里叶参数“原始分布”（raw space） ===
        # 说明：
        # - 这里输出的是未约束的 Normal 分布参数（mean/std），真正的物理参数范围约束在 choose_action/learn 里通过可逆变换实现
        # - 这样可以避免使用 clamp/取模 后再用原 Normal 计算 log_prob 导致的概率不一致问题
        if self.use_fourier:
            # n_harmonics 分布（离散）
            n_logits = self.n_head(features)
            n_logits = torch.clamp(n_logits, min=-10.0, max=10.0)

            # T 的 raw 分布参数（后续用 sigmoid 映射到 [T_min, T_max]）
            T_params = self.T_head(features)
            T_mean = T_params[0:1]  # raw mean
            T_log_std = T_params[1:2].clamp(min=-2.0, max=1.0)
            T_std = torch.exp(T_log_std).clamp(min=0.01, max=2.0)

            # A 的 raw 分布参数（后续用 tanh 映射到 [-1,1]）
            A_params = self.A_head(features).view(self.n_servos, self.max_harmonics, 2)
            A_mean = A_params[:, :, 0]  # raw mean
            A_log_std = A_params[:, :, 1].clamp(min=-2.0, max=1.0)
            A_std = torch.exp(A_log_std).clamp(min=0.01, max=2.0)

            # ω 的 raw 分布参数（后续用 sigmoid 映射到 [ω_min, ω_max]）
            ω_params = self.ω_head(features).view(self.max_harmonics, 2)
            ω_mean = ω_params[:, 0]  # raw mean
            ω_log_std = ω_params[:, 1].clamp(min=-2.0, max=1.0)
            ω_std = torch.exp(ω_log_std).clamp(min=0.01, max=2.0)

            # φ 的 raw 分布参数（后续用 sigmoid 映射到 [0, 2π)）
            φ_params = self.φ_head(features).view(self.n_servos, self.max_harmonics, 2)
            φ_mean = φ_params[:, :, 0]  # raw mean
            φ_log_std = φ_params[:, :, 1].clamp(min=-2.0, max=1.0)
            φ_std = torch.exp(φ_log_std).clamp(min=0.01, max=2.0)
...
class PPO:
    def __init__(self, node_num, env_information, act_dim=1, use_fourier=True, 
        ...
        self.next_states = []
        self.values = []           # 合并存储价值
        # 【新增】存储每条transition对应的 V(s_{t+1})，用于GAE在截断(done=0)时bootstrap
        self.next_values = []
    ...
    def choose_action(self, episode_num, obs, x_graph, explore=None):
        ...
        if self.use_fourier:
            with torch.no_grad():
                ...
                n_idx = n_dist.sample()          # [0, max_harmonics-1]
                n = n_idx + 1                    # [1, max_harmonics]

                # 2) 采样 raw 参数（连续）
                T_dist = Normal(dists['T_mean'], dists['T_std'].clamp(min=0.01))
                A_dist = Normal(dists['A_mean'], dists['A_std'].clamp(min=0.01))
                ω_dist = Normal(dists['ω_mean'], dists['ω_std'].clamp(min=0.01))
                ...
                # 3) 通过可逆变换映射到物理范围（避免 clamp/取模 导致 log_prob 不一致）
                # T ∈ [T_min, T_max]
                T_sig = torch.sigmoid(T_raw)
                T_sample = T_min + (T_max - T_min) * T_sig

                # A ∈ [-1, 1]
                A_sample = torch.tanh(A_raw)

                # ω ∈ [ω_min, ω_max]
                ω_sig = torch.sigmoid(ω_raw)
                ω_sample = ω_min + (ω_max - ω_min) * ω_sig

                # φ ∈ [0, 2π)（相位，弧度）
                φ_sig = torch.sigmoid(φ_raw)
                φ_sample = φ_max * φ_sig

                # 4) 只对前 n 个谐波计算 log_prob（和环境实际用到的维度一致）
                harm_mask = (torch.arange(self.max_harmonics, device=device) < n).float()       # [max_harmonics]
                harm_mask_2d = harm_mask.unsqueeze(0)                                            # [1, max_harmonics]

                # 5) 计算 log_prob（change-of-variables 修正）
                log_prob_n = n_dist.log_prob(n_idx)

                # sigmoid 变换：y = a + (b-a)*sigmoid(x)，|dy/dx|=(b-a)*s*(1-s)
                log_prob_T = T_dist.log_prob(T_raw) - torch.log((T_max - T_min) * T_sig * (1 - T_sig) + eps)
                log_prob_T = log_prob_T.sum()

                # tanh 变换：y=tanh(x)，|dy/dx|=1-tanh(x)^2
                log_prob_A = A_dist.log_prob(A_raw) - torch.log(1 - A_sample.pow(2) + eps)
                log_prob_A = (log_prob_A * harm_mask_2d).sum()

                # ω：sigmoid 映射到 [ω_min, ω_max]
                log_prob_ω = ω_dist.log_prob(ω_raw) - torch.log((ω_max - ω_min) * ω_sig * (1 - ω_sig) + eps)
                log_prob_ω = (log_prob_ω * harm_mask).sum()

                # φ：sigmoid 映射到 [0, 2π)
                log_prob_φ = φ_dist.log_prob(φ_raw) - torch.log(φ_max * φ_sig * (1 - φ_sig) + eps)
                log_prob_φ = (log_prob_φ * harm_mask_2d).sum()

                total_log_prob = log_prob_n + log_prob_T + log_prob_A + log_prob_ω + log_prob_φ

                # 6) 返回：物理参数 + raw 参数（用于 PPO 训练时稳定重算 log_prob）
                action_params = {
                    'n': int(n.cpu().item()),
                    'T': float(T_sample.cpu().item()),
                    'A': A_sample.cpu().numpy(),
                    'ω': ω_sample.cpu().numpy(),
                    'φ': φ_sample.cpu().numpy(),
                    'T_raw': float(T_raw.cpu().item()),
                    'A_raw': A_raw.cpu().numpy(),
                    'ω_raw': ω_raw.cpu().numpy(),
                    'φ_raw': φ_raw.cpu().numpy(),
                }

                return action_params, float(total_log_prob.cpu().item()), float(value.cpu().item()), None
    ...
    @torch.no_grad()
    def evaluate_fourier_log_prob_value(self, obs, x_graph, action_params):
        """【傅里叶模式】在给定 state 上评估固定 action_params 的 log_prob 和 V(s)。

        用途：方案B —— 每条transition都要存对应 state 下的 old_log_prob(s_t,a) 和 V(s_t)。
        """
        if not self.use_fourier:
            raise RuntimeError("evaluate_fourier_log_prob_value 仅在 use_fourier=True 时使用")
        ...
        # 清理 logits/std，保持与 learn() 中一致的数值稳定策略
        ...
        # action_params -> tensor
        n_tensor = torch.tensor(int(action_params['n']) - 1, dtype=torch.long, device=device)
        T_tensor = torch.tensor(float(action_params['T']), dtype=torch.float32, device=device)
        A_tensor = torch.tensor(action_params['A'], dtype=torch.float32, device=device)
        ω_tensor = torch.tensor(action_params['ω'], dtype=torch.float32, device=device)
        φ_tensor = torch.tensor(action_params['φ'], dtype=torch.float32, device=device)
    ...
    @torch.no_grad()
    def evaluate_value(self, obs, x_graph):
        """评估 V(s)（傅里叶/原始模式通用）。"""
        if isinstance(obs, tuple):
            x = obs[0]
            state = obs[1]
        ...
        if self.use_fourier:
            _, value = self.policy(x, state, x_graph)
        else:
            _, value = self.policy(x, state, x_graph)
        return float(value.item())

    def store_transition_catch(self, state, action, reward, next_state, done, value, log_prob,
                               next_value=None, action_raw=None, image=None, angles=None, time_step=None):
        """
        存储经验
        ...
        # next_value可选：老代码不传时，先用0占位（会导致最后一步bootstrap不准）
        self.next_values.append(0.0 if next_value is None else float(next_value))
    ...
    def calculate_advantages(self, rewards, values, dones, next_values=None):
        """
        计算优势函数和回报
        """
        ...
        # next_values: 每条transition对应的 V(s_{t+1})
        if next_values is None:
            # 兼容老调用：没有next_values时，退化为全0（不bootstrap）
            next_values = np.zeros_like(rewards, dtype=np.float32)
        else:
            if len(next_values) != len(rewards):
                raise ValueError("Critical Error: next_values and rewards have different lengths.")
        ...
        for t in reversed(range(len(rewards))):
            # 使用真实的 V(s_{t+1}) bootstrap（done==1时强制为0）
    ...
    def learn(self):
        ...
        all_next_values = self.next_values
        all_log_probs = self.log_probs
        all_dones = self.dones

        # 计算优势函数和回报
        advantages, returns = self.calculate_advantages(all_rewards, all_values, all_dones, next_values=all_next_values)
        if len(advantages) == 0:
            return 0.0
            ...
            for _ in range(self.update_epochs):
                ...
                for start_idx in range(0, len(all_states), self.batch_size):
                    ...
                    for i in range(len(batch_x)):
                        ...
                        # 重新计算log概率（raw space + 可逆变换 + Jacobian 修正）
                        eps = 1e-6
                        T_min = 0.1
                        T_max = self.T_max
                        ω_min = 0.1
                        ...
                        # 1) 构造 raw 分布（注意：forward 已输出 raw mean/std）
                        n_dist = Categorical(logits=n_logits_clean)
                        T_dist = Normal(dists['T_mean'], dists['T_std'].clamp(min=0.01))
                        A_dist = Normal(dists['A_mean'], dists['A_std'].clamp(min=0.01))
                        ω_dist = Normal(dists['ω_mean'], dists['ω_std'].clamp(min=0.01))
                        ...
                        # 2) 取出当时采样的动作（优先用 raw；没有 raw 则反推，兼容旧数据）
                        n_val = int(action_params['n'])
                        n_tensor = torch.tensor(n_val - 1, dtype=torch.long, device=device)
                        ...
                        # T_raw
                        if 'T_raw' in action_params:
                            T_raw = torch.tensor(action_params['T_raw'], dtype=torch.float32, device=device)
                        else:
                            T_val = torch.tensor(action_params['T'], dtype=torch.float32, device=device)
                            z = (T_val - T_min) / (T_max - T_min)
                            T_raw = _logit(z)

                        # A_raw
                        if 'A_raw' in action_params:
                            A_raw = torch.tensor(action_params['A_raw'], dtype=torch.float32, device=device)
                        else:
                            A_val = torch.tensor(action_params['A'], dtype=torch.float32, device=device)
                            A_raw = torch.atanh(torch.clamp(A_val, -0.999999, 0.999999))

                        # ω_raw
                        if 'ω_raw' in action_params:
                            ω_raw = torch.tensor(action_params['ω_raw'], dtype=torch.float32, device=device)
                        else:
                            ω_val = torch.tensor(action_params['ω'], dtype=torch.float32, device=device)
                            z = (ω_val - ω_min) / (ω_max - ω_min)
                            ω_raw = _logit(z)

                        # φ_raw
                        if 'φ_raw' in action_params:
                            φ_raw = torch.tensor(action_params['φ_raw'], dtype=torch.float32, device=device)
                        else:
                            φ_val = torch.tensor(action_params['φ'], dtype=torch.float32, device=device)
                            z = φ_val / φ_max
                            φ_raw = _logit(z)

                        # 3) 变换到物理空间（用于 Jacobian 项）
                        T_sig = torch.sigmoid(T_raw)
                        A_tanh = torch.tanh(A_raw)
                        ω_sig = torch.sigmoid(ω_raw)
                        φ_sig = torch.sigmoid(φ_raw)

                        # 4) 只对前 n 个谐波参与 log_prob（与环境使用维度一致）
                        harm_mask = (torch.arange(self.max_harmonics, device=device) < n_val).float()
                        harm_mask_2d = harm_mask.unsqueeze(0)

                        # 5) change-of-variables 计算 new_log_prob（在 raw 上算 Normal，再减去 log|detJ|）
                        log_prob_n = n_dist.log_prob(n_tensor)

                        log_prob_T = T_dist.log_prob(T_raw) - torch.log((T_max - T_min) * T_sig * (1 - T_sig) + eps)
                        log_prob_T = log_prob_T.sum()

                        log_prob_A = A_dist.log_prob(A_raw) - torch.log(1 - A_tanh.pow(2) + eps)
                        log_prob_A = (log_prob_A * harm_mask_2d).sum()
                        ...
                        new_log_prob = log_prob_n + log_prob_T + log_prob_A + log_prob_ω + log_prob_φ
                        ...
                        # 熵（简单做法：用 raw Normal 的熵 + n 的熵；对高维项取 mean 防止量级爆炸）
                        entropy += (
                            n_dist.entropy() +
                            T_dist.entropy().sum() +
                            A_dist.entropy().mean() +
