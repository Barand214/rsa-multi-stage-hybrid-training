...
> 当前 WaveGrad 目录已独立出入口、配置、日志和 checkpoint；策略核心是连续噪声等级的 WaveGrad-style 动作去噪模型。当前版本已移除额外 Q 网络和动作引导模块，外层 Webots 交互、奖励、安全过滤和 replay 保持原工程风格。
...
4. 训练时使用奖励加权 diffusion loss，同时加入 value head 作为回报估计辅助信号。
...
其中 `value_head` 不参与 PPO ratio/clip，只作为回报估计辅助 diffusion 训练。
...
3. WaveGrad denoiser 从噪声中采样一个动作
4. 训练模式下加少量高斯探索噪声
5. 动作 clip 到配置范围
```
...
2. value head：辅助估计 discounted return，用于稳定回报加权。
...
4. diffusion_loss / value_loss 是否逐渐稳定
